...
void CheckCu(CUresult result, const char* message) {
  if (result != CUDA_SUCCESS) {
    ...
    throw std::runtime_error(
        std::string(message) + ": " +
        (error_name != nullptr ? error_name : "CUDA driver error") + " (" +
...
PYBIND11_MODULE(_turbobus, m) {
  ...
  m.def("export_device_ipc_mapping",
        [](std::uintptr_t device_ptr) {
          ...
          CheckCu(cuMemGetAddressRange(
                      &allocation_base,
                      &allocation_bytes,
                      static_cast<CUdeviceptr>(device_ptr)),
                  "cuMemGetAddressRange failed");
          const auto base_ptr = static_cast<std::uintptr_t>(allocation_base);
          if (base_ptr == 0) {
            throw std::runtime_error(
                "cuMemGetAddressRange returned a null allocation base");
          }
          if (base_ptr > device_ptr) {
            throw std::runtime_error(
                "cuMemGetAddressRange returned an invalid allocation base");
          }
          cudaIpcMemHandle_t handle;
          CheckCuda(cudaIpcGetMemHandle(
                        &handle,
                        reinterpret_cast<void*>(static_cast<uintptr_t>(allocation_base))),
