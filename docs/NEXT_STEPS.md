...
- Adapter transfer context and offload / inference adapter creation are owned
  by `TurboBusRuntimeSession`.
...
feeds `fetch_h2d()` / `offload_d2h()` and the offload / inference adapters.
